#include <stdio.h>
    ...
    */
    ...
    Ability *init_enemy_abilities(){
        Ability *enemies_abs = malloc(sizeof(Ability));
    }
    ...
    Scenario *init_scenario(){ // definimos la función del escenario
        ...
        char *descript_scenario_6 =  "Niflheim, el reino de la niebla eterna y el frío implacable. Aquí, las almas de los malvados son castigadas en un eterno invierno, mientras las sombras acechan entre la niebla espesa.";
            ...
            scenarios->decisions.options[0].numEnemies = 2; // Número aleatorio de 0 a 3
    ...
    Scenario *init_scenario2(){ // definimos la función del escenario
        Scenario *scenarios = malloc(sizeof(Scenario)); // reservamos memoria para los  escenarios
        if (scenarios == NULL){
            printf("Hay un error de memoria");
            exit(1);
        ...
        char *descript_scenario_2 = "Mithra es un valle sagrado, donde los antiguos dioses dejaron su huella en forma de templos y estatuas. Las leyendas cuentan que aquí es donde el mundo de los dioses se encuentra con el de los mortales.";
            ...
            strcpy(scenarios->name, name_scenarios_2);
            strcpy(scenarios->description, descript_scenario_2);
            scenarios->decisions.numOptions=2;
            char q[] = "Te encuentras con un gran templo, que vas a hacer?\n";
            strcpy(scenarios->decisions.question, q);
            ...
            printf("\n[DECISION] -> %s",scenarios->decisions.question);
            printf("\nNumero de opciones disponibles -> %i",scenarios->decisions.numOptions);
            char *response_1 = "Decides entrar al templo y enfrentarte a los peligros de su interior.";
            char *response_2 = "Optas por no entrar al templo, rodearlo, quizas la opcion mas segura, pero cobarde.";
            strcpy(scenarios->decisions.options[0].response, response_1);
            strcpy(scenarios->decisions.options[1].response, response_2);
        ...
        Scenario *init_scenario3(){ // definimos la función del escenario
        Scenario *scenarios = malloc(sizeof(Scenario)); // reservamos memoria para los  escenarios
        if (scenarios == NULL){
            printf("Hay un error de memoria");
            exit(1);
        ...
        char *descript_scenario_3 = "Las Ruinas de Vandal son un lugar olvidado por el tiempo, donde antaño se alzaba una gran ciudad. Ahora, solo quedan ruinas cubiertas por la maleza y el misterio de un pasado perdido.";
            ...
            strcpy(scenarios->name, name_scenarios_3);
            strcpy(scenarios->description, descript_scenario_3);
            scenarios->decisions.numOptions=2;
            char q[] = "Llegas a Vandal, la antigua gran civilizacion, que deseas hacer?\n";
            strcpy(scenarios->decisions.question, q);
            ...
            printf("\n[DECISION] -> %s",scenarios->decisions.question);
            printf("\nNumero de opciones disponibles -> %i",scenarios->decisions.numOptions);
            char *response_1 = "Decides explorar las ruinas de la gran civilizacion hasta llegar a un edificio parecido a un castillo, al cual entras.";
            char *response_2 = "Decides avanzar con prisa y sin mirar atras, evitando cualquier tipo de exploracion innecesaria intentando evitar el contacto con enemigos";
            strcpy(scenarios->decisions.options[0].response, response_1);
            strcpy(scenarios->decisions.options[1].response, response_2);
    ...
    Scenario *init_scenario4(){ // definimos la función del escenario
        Scenario *scenarios = malloc(sizeof(Scenario)); // reservamos memoria para los  escenarios
        if (scenarios == NULL){
            printf("Hay un error de memoria");
            exit(1);
        ...
        char *descript_scenario_4 = "En River lake, el agua cristalina refleja el cielo eterno de Asgard. Este lugar tranquilo esconde secretos bajo su superficie serena, donde los ríos fluyen hacia los reinos de los dioses.";
            ...
            strcpy(scenarios->name, name_scenarios_4);
            strcpy(scenarios->description, descript_scenario_4);
            scenarios->decisions.numOptions=2;
            char q[] = "Llegas a un lago cristalino, que quieres hacer?\n";
            strcpy(scenarios->decisions.question, q);
            ...
            printf("\n[DECISION] -> %s",scenarios->decisions.question);
            printf("\nNumero de opciones disponibles -> %i",scenarios->decisions.numOptions);
            char *response_1 = "Decides descansar tras tanto camino recorrido, bebes un poco de agua y prosigues tu camino.";
            char *response_2 = "Decides quedarte aqui, crees que ya has recorrido y luchado bastante. (THE END)";
            strcpy(scenarios->decisions.options[0].response, response_1);
            strcpy(scenarios->decisions.options[1].response, response_2);

            // Generar un número aleatorio de enemigos para la opción 1
            srand(time(NULL)); // Inicializar el generador de números aleatorios
            scenarios->decisions.options[0].numEnemies = 0; // Número aleatorio de 0 a 3
    ...
    Scenario *init_scenario5(){ // definimos la función del escenario
        Scenario *scenarios = malloc(sizeof(Scenario)); // reservamos memoria para los  escenarios
        if (scenarios == NULL){
            printf("Hay un error de memoria");
            exit(1);
        ...
        char *descript_scenario_5 = "Yggdrasil, el Árbol del Mundo, se alza imponente ante ti, sus ramas se extienden hacia los cielos y las raíces se hunden en las profundidades de los nueve mundos. Es el eje que sostiene la existencia misma.";
            ...
            strcpy(scenarios->name, name_scenarios_5);
            strcpy(scenarios->description, descript_scenario_5);
            scenarios->decisions.numOptions=2;
            char q[] = "Ultima parada de tu aventura Yggdrasil el gran arbol del mundo ante ti poderosos enemigos, que deseas hacer?\n";
            strcpy(scenarios->decisions.question, q);
            ...
            printf("\n[DECISION] -> %s",scenarios->decisions.question);
            printf("\nNumero de opciones disponibles -> %i",scenarios->decisions.numOptions);
            char *response_1 = "Decides avanzar con valentia hacia el frente y enfrentarte a los enemigos para ser rey del mundo. (THE END)";
            char *response_2 = "Decides evadir los combates y acceder dentro del arbol para ser su guardian. (THE END)";
            strcpy(scenarios->decisions.options[0].response, response_1);
            strcpy(scenarios->decisions.options[1].response, response_2);

            // Generar un número aleatorio de enemigos para la opción 1
            srand(time(NULL)); // Inicializar el generador de números aleatorios
            scenarios->decisions.options[0].numEnemies = 1; // Número aleatorio de 1 a 3

            // Asignar enemigos aleatorios a la opción 1
            scenarios->decisions.options[0].enemies[0] = enemies[2];
            printf("\n-------------------------------------------------------------------------------------------");
            printf("\n[OPCION 1] -> %s",scenarios->decisions.options[0].response);
            printf("\nNumero de enemigos en opcion 1 -> %d", scenarios->decisions.options[0].numEnemies);
            for (int i = 0; i < scenarios->decisions.options[0].numEnemies; i++) {
